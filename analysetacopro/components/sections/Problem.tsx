...
export function Problem() {
  return (
    <Section className="bg-brand-soft/40">
      ...
      <div className="mt-14 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
        {PROBLEMS.map((p, i) => (
          <Reveal key={p.title} delay={i * 70}>
            <div className="group h-full rounded-3xl border border-brand/5 bg-white p-7 shadow-card transition-all duration-300 hover:-translate-y-1 hover:shadow-soft-lg">
              <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-brand-soft text-brand transition-colors group-hover:bg-brand-gradient group-hover:text-white">
      ...
      <Reveal delay={150}>
        <p className="mx-auto mt-12 max-w-2xl text-center text-lg font-semibold text-ink">
